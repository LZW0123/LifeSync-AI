...
def email_advice_with_ai(data, ai_version, present_location, user_career, local_time, schedule_prompt=""):
    ...
    try:
        # 如果今日有已完成任务，准备今日完成的任务信息
        completed_today_summary = "\n".join([
            f"- {task['Name']}: {task['Description']} (进展：{task['Progress']})"
            for task in data.get("completed_today_tasks", [])
        ]) if data.get("completed_today_tasks") else "今日没有已完成的任务。"
        ...
        你要做的事情 三：
        如果有任务，根据每一个任务的当前进展情况以及对每个任务的紧急程度进行排序。
        ...
        2. 明日任务提醒和建议（越详细越好，日程可以告诉我时间，任务不用规定时间）：总结当天未完成的任务，并提醒明天要做的任务。用小字备注任务详情、当前进展和注意事项(一定要给出来)，为用户写提示以更好地理解任务。
        3. 今日已完成的任务总结：汇报今天完成的任务，任务详情和注意事项要清晰显示。
        4. 未来任务和日程提醒和建议（越详细越好）：如果未来任务比较紧急，可以提醒未来要做的任务及其时间。同时用小字备注任务详情、当前进展和注意事项（一定要给出来），为用户写提示以更好地理解任务。
        ...
        print("Generated.\n")
        return re.sub(r'<body>|</body>|```html?|```', '',
                      response.choices[0].message.content.strip() if response.choices else "No guidance provided.")
