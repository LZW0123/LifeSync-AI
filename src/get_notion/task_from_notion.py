    ...
    try:
        ...
        for row in results["results"]:
            if 'date' in row['properties']['Date'] and row['properties']['Date']['date']:
                ...
                # 获取紧急程度
                urgency_level = row['properties']['紧急程度']['select']['name'] if '紧急程度' in row['properties'] and \
                                                                                   row['properties']['紧急程度'][
                                                                                       'select'] else 'NA'
                ...
                if 'rich_text' in row['properties']['Description'] and row['properties']['Description']['rich_text']:
                    description = ''.join(
                        [part['text']['content'] for part in row['properties']['Description']['rich_text']])
                    # 清理特殊字符
                ...
                if end_datetime and end_datetime.date() < today:
                    ...
                    tasks["today_due"].append(task)
                elif start_datetime and start_datetime.date() <= today and (
                        not end_datetime or end_datetime.date() > today):
